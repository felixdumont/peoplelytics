...
function generate_paths() {
  ...
  logProgress('Starting to write');
  try {
    
    //const stringifyStream = json.createStringifyStream({
    //    body: tripsWithLocations
    //});
 
    //stringifyStream.on('paths.json', function(strChunk) {
     // => BIG_POJO will be sent out in JSON chunks as the object is traversed
    //});
    
    const output = JSON.stringify(tripsWithLocations);
    logProgress('Writing, len: ' + output.length);
    fs.writeFileSync('paths.json', output);
    
    
    //var wstream = fs.createWriteStream('paths.json');
    //wstream.write(JSON.stringify(tripsWithLocations));
    //wstream.end();
